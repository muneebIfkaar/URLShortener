...
public enum DBConnectionPool {
	...
	private void init() throws Exception {
		...
		log.info("Usernamae: {}, pwd: {}, HostName: {}, port: {}, dbName: {}",userName, password, hostName, port, dbName);
	}
	
	private void loadPropertiesFile() throws Exception{
		PROP = new Properties();
		//PROP.load(new FileInputStream(getClass().getResourceAsStream("/db.conf")));
		
		init();
	}

	private String userName = "root"; //= Global.INSTANCE.getPropertyValue("db.cab_service.username");
	private String password = "root"; //= Global.INSTANCE.getPropertyValue("db.cab_service.password");
	private String hostName = "localhost"; //= Global.INSTANCE.getPropertyValue("db.cab_service.hostname");
	private String port		= "3306"; //= Global.INSTANCE.getPropertyValue("db.cab_service.port");
	private String dbName	= "urls"; //= Global.INSTANCE.getPropertyValue("db.cab_service.dbname");
